...
| `ShipDismantle` | Dismantles ships (raft, karve, longship) with the hammer without a workbench nearby, e.g. on a remote shore. `Repair` (on by default) also lets the hammer repair ships without a workbench. Every other vanilla rule stays: wards, no-build zones, "can't remove now" while someone is aboard a raft, and the materials and cargo drop at the ship's position as usual. Client only: neither the server nor the other players need the mod. |
